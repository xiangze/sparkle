...
/-- Proposition: MAC with zero activation produces unchanged partial sum. -/
def mac_zero_act (w : BitVec 8) (psum : BitVec 32) : Prop :=
  macSpec (0#8) w psum = psum

/-- Proposition: MAC multiplication component can be written in swapped operand order.
    (This records the intended commutativity property as a proposition.) -/
def mac_mul_comm (a w : BitVec 8) (psum : BitVec 32) : Prop :=
  macSpec a w psum = psum + BitVec.zeroExtend 32 w * BitVec.zeroExtend 32 a
